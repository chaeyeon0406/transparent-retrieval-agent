...
Request access at adni.loni.usc.edu/data-samples/access-data (typically ~1 week for approval).
Download T1-weighted MRI and associated clinical variables (Age, MMSE, CDR-Global) for the ADNI1 and ADNI2 cohorts.
Extract the 75-dimensional fused feature vector (25 DCT + 21 DWT + 26 LBP image-texture features, plus 3 clinical variables) as described in Sec. 2.1 of the paper, and format as img_0...img_71, AGE, MMSCORE, CDGLOBAL, RID, Target columns.
Repository structure
Script	Produces
svd_svm.py	Grid search over SVD dimensionality and SVM $(C,\gamma)$; fusion classification accuracy (Sec. 3)
...
Results summary
P@10 before → after	P@10 before → after
Feedback	Classification accuracy
ADNI1 ($n{=}699$)	0.828 → 0.948	93.24%
ADNI2 ($n{=}840$)	0.832 → 0.884	95.94%
