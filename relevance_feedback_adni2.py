#!/usr/bin/env python3
"""
relevance_feedback_adni2.py
Relevance feedback evaluation on the ADNI2 cohort, replicating the same
protocol as relevance_feedback_adni1.py (Agarwal & Mostafa, 2011, Sec. 2-3
of the paper).
...
Applying feedback to the full result set would trivially reach P@10=1.0,
since the retrieval pool itself already contains all same-class matches.
"""
...
OUT_RF           = 'adni2_validation/features/relevance_feedback_results_adni2.csv'
...
for qi, q_idx in enumerate(query_idxs):
    ...
    # (B) Inter-session re-ranking (accumulated class profile)
...
print()
print("=" * 65)
print(f"AVERAGE PRECISION (25 queries, feedback_window={FEEDBACK_WINDOW})")
